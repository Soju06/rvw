...
## Synthesis telemetry (2026-09-09)

The ordinary pipeline carries synthesis status, model, reasoning effort, aggregate attempt wall and tool counts into summary.json (and run health for replay). Missing historical synthesis resolves to fallback:not-run. A failed explanatory stage does not change policy or review status. The existing runtime handles egress, process cleanup and the no-output watchdog. The synthesis-specific budget and fidelity rules are in the synthesis capability. The stale normative max-effort wording was corrected to high to match the already-shipped 0.20.0 default; no runtime default changed.
