...
- Finding agreement is discovery evidence: the numerator remains the distinct producing replica count, while the denominator is recovered from exact persisted `(replica, chunk)` coverage rather than adjudication votes. Coverage-free historical artifacts retain the old `/3` rendering because neither agreement nor vote counts can reconstruct the configured discovery count safely.
