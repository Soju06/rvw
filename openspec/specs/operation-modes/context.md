...
Four consecutive production App runs on the consuming repository at a 900 s deadline showed the `correctness`, `hygiene`, and `test-integrity` lanes hitting the deadline on every attempt while stating that every changed region was covered and a final evidence pass was underway, then continuing to call tools: 35 to 94 tool calls per dead lane against 5 to 19 for lanes that finished, unchanged by the 0.15.0 prompt budget contract. The owner wants to A/B the reasoning effort (`medium`, `high`) and the model (a `gpt-6` variant) against the packaged `gpt-5.6-sol` / `max` before touching lane prompts, which needs a per-run override with no code change on both surfaces. The CLI side of that is the requirement "Runtime-executing commands expose the Codex model and reasoning effort" in [spec.md](spec.md); the resolver, enum, and artifact recording are described in the runtime-contract context.
