...
## Replica watchdog (2026-09-08)

Adjudication replicas inherit the runtime no-output watchdog and nothing else about the stage changes: an INVALID replica casts no vote, and the pass retries only when every replica is INVALID. On the measured production run the initial adjudication wave's three tool-less replicas recorded walls of 160.06 s, 546.58 s, and 900.12 s; the 900.12 s replica produced no byte after its prompt echo and was killed by the 900 s deadline, so the wave wall was the deadline and the two valid votes waited for it. With the 660 s default the same wave ends when the silent replica is killed at 660 s, voting proceeds on the two valid outputs, no retry runs, and the wave wall is about 660 s: the killed replica's wall is `N` plus at most one 2 s poll interval and the terminate-and-reap grace (up to 10 s for a runtime that ignores TERM), so `max(160.06, 546.58, ~660)` instead of the 900 s deadline. The default sits above the longest legitimate silence measured so far (546.6 s), so a replica that is still thinking is not killed by it; when every replica is killed, the existing single all-invalid retry carries the `no_output_after` reasons.
