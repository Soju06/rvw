...
- Every member runs the existing DISCOVER, MERGE, ADJUDICATE, and REPORT
  pipeline in a disposable checkout. Stack orchestration adds sequencing and
  lineage work rather than maintaining a second review implementation.
...
- Presence has a distinct strict schema: PRESENT and ABSENT require current
  source evidence, missing items are UNCERTAIN, an all-invalid wave retries
  once, and uncertain residue receives one expanded-context pass.
- Publication is one body-only COMMENT on the tip. Cross-head claims do not have
  ...
  duplicated or stale review state.
...
- A member that moves during execution leaves partial member-run and lineage
  artifacts but no completed stack report.
